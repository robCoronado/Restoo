...
import MapRestaurant from '../../components/Restaurants/MapRestaurants'
...
export default function Restaurants({ navigation, route }) {
    ...
    const addFavorite = async() => {
        if (!userLogged) {
            toastRef.current.show("Para agregar el restaurante a favoritos debes iniciar sesion", 3000)
            return
        }
        ...
        if (response.statusResponse) {
            setIsFavorite(true)
            toastRef.current.show("El Restaurante se ha añadido a favoritos.", 3000)
        } else {
